...
public abstract class AbstractDAO<C0 extends local.tin.tests.jpa.workshop.model.domain.interfaces.IIdentifiable, C1 extends local.tin.tests.jpa.workshop.model.data.interfaces.IIdentifiable>  implements IAbstractDAO<C0> {
    ...
    protected C0 updateDomainObjectId(C1 dataObject, C0 domainObject) throws DAOException {
        if (dataObject.getId() instanceof local.tin.tests.jpa.workshop.model.data.interfaces.IEmbeddable) {
            domainObject.setId(getCompositedId((local.tin.tests.jpa.workshop.model.data.interfaces.IEmbeddable) dataObject.getId()));
        } else {
    ...
    protected C1 updateDataObjectId(C1 dataObject, C0 domainObject) throws DAOException {
        if (domainObject.getId() instanceof local.tin.tests.jpa.workshop.model.domain.interfaces.ICompositeId) {
            dataObject.setId(getEmmbeddedId((local.tin.tests.jpa.workshop.model.domain.interfaces.ICompositeId) domainObject.getId()));
        } else {
