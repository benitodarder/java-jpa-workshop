    ...
    public static void main(String[] args) throws IOException, DAOException, Exception {
        ...
        if (args.length == 1) {
            ...
            if (master != null && master.getTotalCommissionLocal() != null && master.getTotalCommissionSystem() != null && master.getTotalCommissionTransaction() != null) {
                ...
                bookingCommissionItem = secondaryDAO.create(bookingCommissionItem);
                BillingItemDAO billingItemDAO = (BillingItemDAO) CopyValuesDAOFactory.getInstance().getDAO(BillingItem.class);
                BillingItem billingItemFromProductItem = billingItemDAO.retrieveByBookingItem((Integer) master.getId()).iterator().next();
                BillingItem billingItem = new BillingItem();
                billingItem.setAmountLocal(bookingCommissionItem.getTotalCommissionLocal());
                billingItem.setAmountSystem(bookingCommissionItem.getTotalCommissionSystem());
                billingItem.setAmountTransaction(bookingCommissionItem.getTotalCommissionTransaction());
                billingItem.setBillingBusinessType("PURCHASE_SALE");
                billingItem.setBillingItemSource("AGENT_COMMISSION");
                billingItem.setBillingItemType("INCOME");
                billingItem.setBookingId(master.getBooking());
                billingItem.setBookingItemId((Integer) bookingCommissionItem.getId());
                billingItem.setCurrencyCodeLocal(bookingCommissionItem.getCurrencyCodeLocal());
                billingItem.setCurrencyCodeSystem(bookingCommissionItem.getCurrencyCodeSystem());
                billingItem.setCurrencyCodeTransaction(bookingCommissionItem.getCurrencyCodeTransaction());
                billingItem.setCustomerId(master.getSellerCompanyId());
                billingItem.setGdStatus(billingItemFromProductItem.getGdStatus());
                billingItem.setGdStatusFlowType(billingItemFromProductItem.getGdStatusFlowType());
                billingItemDAO.create(billingItem);
                billingItem.setBillingItemType("COST");
                billingItem.setCompanyId(master.getSellerCompanyId());
                billingItem.setCustomerId(null);
